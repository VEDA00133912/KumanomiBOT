  ...
  async getRandomSongs(interaction, commandName, folder, option, count, embedColor) {
    ...
    try {
      ...
      if (songList.length === 0) {
        return interaction.editReply('<:error:1299263288797827185> 指定されたオプションに対応する曲が見つかりませんでした。');
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .setTimestamp()
        .setFooter({ text: `Kumanomi | ${commandName}`, iconURL: interaction.client.user.displayAvatarURL() })
        .setColor(embedColor);
