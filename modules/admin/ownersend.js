...
module.exports = {
  ...
  async execute(message) {
    ...
    if (message.content.startsWith('^send')) {
      ...
      if (!messageId) {
        return message.reply({ embeds: [new EmbedBuilder().setColor('Red').setDescription('<:error:1302169165905526805> メッセージIDを指定してください。')] });
      }
      ...
      try {
            ...
            new EmbedBuilder()
              ...
              .setDescription(
                failedGuilds.length > 0
                  ? `<:check:1302169183110565958> 成功: ${successCount}人\n失敗: ${failedGuilds.length}人\n**失敗したギルド**\n${failedGuilds.join(', ')}`
                  : `<:check:1302169183110565958> 全てのオーナーにメッセージ内容を送信しました。成功: ${successCount}人`
              )
        ...
        await message.reply({
          embeds: [
            new EmbedBuilder()
              .setColor('Red')
              .setDescription('<:error:1302169165905526805> 指定されたメッセージIDが無効です。')
          ]
        });
      }
    }
